...
1. [OpenClaw 两个 Plugin Slot 的区别](#1-openclaw-两个-plugin-slot-的区别)
2. [memory-core 在企业多 Agent 场景的局限性](#2-memory-core-在企业多-agent-场景的局限性)
3. [三层插件协同 — 提示词构建流程](#3-三层插件协同--提示词构建流程)
4. [memory-agentcore 接口设计与独特功能](#4-memory-agentcore-接口设计与独特功能)
5. [多 Agent 共享 Memory 详解](#5-多-agent-共享-memory-详解)

---

## 1. OpenClaw 两个 Plugin Slot 的区别
...
## 2. memory-core 在企业多 Agent 场景的局限性
...
│  │     hook: before_agent_start -> { prependContext }            │   │
...
### 3.2 lossless-claw 与 Memory Plugin 的区别和协同
...
用户消息到达
    ...
    |
    +---> before_agent_start hook (memory-agentcore):
...
**before_agent_start (Auto-Recall)** — `src/index.ts:127-209`:

> **注意**：`before_agent_start` 在 OpenClaw 插件文档中标记为 legacy compatibility hook（"may run in either phase"，即执行时机不确定），推荐使用 `before_prompt_build`。memory-agentcore 目前仍使用 `before_agent_start` 以兼容更多 OpenClaw 版本（插件发布时 `before_prompt_build` 尚不普及）。未来版本可能迁移。
...
| 文件 | 用途 | 本文引用章节 |
|------|------|-------------|
| `src/index.ts` | 插件注册、hooks、CLI | 3, 4.1, 4.2, 4.3 |
| `src/client.ts` | AWS SDK 调用 | 4.4 |
| `src/scopes.ts` | 命名空间映射与访问控制 | 5.1, 5.2, 5.5 |
| `src/config.ts` | 配置解析 | 4.3 |
| `src/file-sync.ts` | 文件同步 | 4.5 |
| `src/tools/recall.ts` | 语义搜索工具 | 4.2 |
| `src/tools/store.ts` | 存储工具 | 4.2 |
| `src/tools/forget.ts` | 删除工具 | 4.2 |
| `src/tools/correct.ts` | 更正工具 | 4.2 |
| `src/tools/search.ts` | 列表/过滤工具 | 4.2 |
| `src/tools/episodes.ts` | 情境记忆工具 | 4.2, 5.5 |
| `src/tools/share.ts` | 跨命名空间共享工具 | 4.2, 5.6 |
| `src/tools/stats.ts` | 统计工具 | 4.2 |
| `README.md` | 项目说明 | 2, 3.3, 5.4 |
| `skills/agentcore-memory-guide/SKILL.md` | 使用指南 | 4.2 |
| OpenClaw memory 文档 | 内置 memory 文档（"Memory files" / "Vector memory search" / "Additional memory paths" 等节） | 1, 2, 3.2, 3.3 |
| OpenClaw 插件文档 | 插件系统文档（"Plugin slots" / "Bundled plugins" / "Context engine plugins" 等节） | 1, 4.1 |
| OpenClaw agent loop 文档 | Agent 运行时生命周期文档 | 3 |
| OpenClaw 上下文文档 | Context Engine 概念（"How it works" / "System prompt addition" 等节） | 1 |
| lossless-claw `architecture.md` | DAG 摘要架构 | 3.1, 3.2, 3.3 |
| lossless-claw `agent-tools.md` | Agent 工具文档 | 3.1, 3.2, 3.3 |
