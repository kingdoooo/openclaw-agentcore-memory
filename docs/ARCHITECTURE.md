...
员工 Agent（actorId = agentId）默认不包含 `/users/*` 命名空间。两种授权方式：

**精确授权**（推荐）：`user:ou_xxx` 展开为主命名空间 + 全部策略命名空间（与 `agent:` 行为一致）：

```json5
{ "scopes": { "agentAccess": { "employee-agent": ["user:ou_xxx"] } } }
// → /users/ou_xxx + /semantic/ou_xxx + /episodic/ou_xxx + /preferences/ou_xxx + /summary/ou_xxx
```

也支持单策略精确授权：`user:ou_xxx:semantic` → 只给 `/semantic/ou_xxx`。

**通配符授权**：`user:*` 通过前缀匹配授权所有 `/users/` 命名空间（不展开策略，因为策略命名空间路径不区分 user/agent ID）：

```json5
{ "scopes": { "agentAccess": { "employee-agent": ["user:*"] } } }
// → /users/* 前缀匹配（resolveWildcardPrefixes）
```

> **ID 碰撞假设**：策略命名空间 `/semantic/{id}` 不区分 user/agent，依赖 ID 格式不碰撞。飞书场景下 user ID 为 `ou_xxx` 格式，agent ID 为自定义名称，天然不碰撞。
